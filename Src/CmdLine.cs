...
class CmdLine : ICommandLineValidatable
{
    ...
    [Option("-t", "--to")]
    [DocumentationRhoML("""
        {h}Zero or more path to be used as mirror destination.{}
    ...
    [Option("-l", "--log")]
    [DocumentationRhoML("""
        {h}Specifies log file directory.{}
    ...
    [Option("-i", "--ignore")]
    [DocumentationRhoML("""
        {h}Specifies one or more source paths to ignore.{}
    ...
    [DocumentationRhoML("""
        ...
        A blank file is created if this file does not exist. The mirror and ignore paths may be specified in settings, on the command line, or both.
        """)]
    ...
    public ConsoleColoredString Validate()
    {
        ...
        if (SettingsPath == null && FromPath.Length == 0)
            return CommandLineParser.Colorize(RhoML.Parse("At least one {option}--from{}/{option}--to{} pair is required if {option}--settings{} is not specified."));
        return null;
    }
